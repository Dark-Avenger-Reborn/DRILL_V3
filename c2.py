...
#{uuid.uuid4()}"""
        ...
        if os_name == "Windows":
            ...
            result = subprocess.run(f'docker run --platform linux/amd64 --volume "$(pwd):/src/" batonogov/pyinstaller-linux-slim:latest "pyinstaller -F --onefile --windowed --hidden-import=pty --hide-console hide-early {payload_file_name}.py"', shell=True, capture_output=True)
            print(result.stdout.decode(), result.stderr.decode())  # Print output for debugging
