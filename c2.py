...
#{uuid.uuid4()}"""
        ...
        if os_name == "Windows":
            result = subprocess.run(
                f'docker run --platform linux/amd64 --env DISPLAY=$DISPLAY --volume "$(pwd):/src/" darkavengerreborn/pyinstaller-windows:latest "pyinstaller -F --onefile --windowed --icon=icon.ico --hidden-import=pypiwin32 --hidden-import=pycryptodome --hidden-import=audioop-lts --hide-console hide-early {payload_file_name}.py"',
                shell=True,
                capture_output=True,
            )
            print(
