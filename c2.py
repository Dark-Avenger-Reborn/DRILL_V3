...
#{uuid.uuid4()}"""
        ...
        if os_name == "Windows":
            result = subprocess.run(f'docker run --volume "$(pwd):/src/" batonogov/pyinstaller-windows:latest "pyinstaller -F --onefile --hide-console hide-early {payload_file_name}.py"', shell=True, capture_output=True)
